...
export default function Wjogger(){
    ...
    async function fetchdata(){
        ...
        try{
            ...
            const womenjogger = res?.data?.filter(product => product.catagories === "Women's Joggers")
            setWomenjogger(womenjogger)
            setLoading(false)
        }catch(err){
            setErr(true)
            setLoading(false)
    ...
    if (loading) {
      ...
      if (err) {
        ...
        return (
          <Box >
          <Heading>Women's Joggers </Heading>
          <Grid templateColumns="repeat(2,1fr)" gap={5}>
            {womenjogger.map(product => (
              <Box key={product.id}>
